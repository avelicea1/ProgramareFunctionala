...
monthsBetween dA dB =
    case (month dA, month dB) of
        (Just m1, Just m2) ->
            Just (abs(12 * (year dB - year dA) + monthToInt m2 - monthToInt m1))
        (Nothing, Nothing) ->
            Just (abs (year dB - year dA) * 12)
        _ ->
            Nothing
    -- Nothing
...
compare (Date d1) (Date d2) =
    chainCompare 
        (Basics.compare
                (Maybe.withDefault 21 <| Maybe.map monthToInt <| d1.month)
                (Maybe.withDefault 21 <| Maybe.map monthToInt <| d2.month))
...
dateToString (Date d) = 
    Maybe.withDefault "" (Maybe.map (\x -> monthToString x) d.month) ++ " " ++ String.fromInt d.year ++ " " 
