...
var $author$project$Lists$take = F2(
	function (n, l) {
		if (n <= 0) {
			return _List_Nil;
		} else {
			if (!l.b) {
				...
				return A2(
					$elm$core$List$cons,
					x,
					A2($author$project$Lists$take, n - 1, xs));
			}
		}
	});
...
var $author$project$FirstTest$emptyListTakeTest = A2(
	$elm_explorations$test$Test$test,
	'Take for an empty list returns the empty list',
	function (_v0) {
		return A2(
			$elm_explorations$test$Expect$equal,
			_List_Nil,
			A2($author$project$Lists$take, 1, _List_Nil));
	});
...
var $author$project$Lists$drop = F2(
	function (n, l) {
		drop:
		while (true) {
			if (n <= 0) {
...
var $author$project$OrganizedTests$listTests = A2(
		...
		[
			...
			_List_fromArray(
				[
					...
					A2(
					$elm_explorations$test$Test$test,
					'Take 0 returns the empty list for any list',
					function (_v1) {
						return A2(
							$elm_explorations$test$Expect$equal,
							_List_Nil,
							A2(
								$author$project$Lists$take,
								0,
								_List_fromArray(
									[1, 2])));
					})
			...
			_List_fromArray(
				[
					A2(
					$elm_explorations$test$Test$test,
					'Drop for an empty list returns the empty list',
					function (_v2) {
						return A2(
							$elm_explorations$test$Expect$equal,
							_List_Nil,
							A2($author$project$Lists$drop, 1, _List_Nil));
					}),
					A2(
					$elm_explorations$test$Test$test,
					'Drop 0 returns the original list for any list',
					function (_v3) {
						return A2(
							$elm_explorations$test$Expect$equal,
							_List_fromArray(
...
var $author$project$Date$Apr = {$: 'Apr'};
var $author$project$Date$Feb = {$: 'Feb'};
var $author$project$Date$Date = function (a) {
	return {$: 'Date', a: a};
};
var $author$project$Date$daysInMonth = function (month) {
	switch (month.$) {
		case 'Jan':
			return 31;
		case 'Feb':
...
var $author$project$Date$createDate = F3(
	function (day, month, year) {
		var lastDay = $author$project$Date$daysInMonth(month);
		var between = F3(
			function (start, end, num) {
				return (_Utils_cmp(start, num) < 1) && (_Utils_cmp(num, end) < 1);
			});
		return (!A3(between, 1, lastDay, day)) ? $elm$core$Maybe$Nothing : ((!A3(between, 1970, 3000, year)) ? $elm$core$Maybe$Nothing : $elm$core$Maybe$Just(
		...
		[
			...
			_List_fromArray(
				[
					A2(
					$elm_explorations$test$Test$test,
					'createDate fails if the date is not between 1970 or 3000',
					function (_v0) {
						return A2(
							$elm_explorations$test$Expect$equal,
							$elm$core$Maybe$Nothing,
							A3($author$project$Date$createDate, 14, $author$project$Date$Apr, 1800));
					}),
					A2(
					$elm_explorations$test$Test$test,
					'createDate fails if the day is not between 1 and 28 for February',
					function (_v1) {
						return A2(
							$elm_explorations$test$Expect$equal,
							$elm$core$Maybe$Nothing,
							A3($author$project$Date$createDate, 30, $author$project$Date$Feb, 2001));
					})
				]))
		]));
...
var $author$project$Exercises$usersWithPhoneNumbers = function (users) {
	return A2(
		...
		A2(
			$elm$core$List$filter,
			function (user) {
				return !_Utils_eq(user.details.phoneNumber, $elm$core$Maybe$Nothing);
			},
			users));
};
		...
		[
			...
			_List_fromArray(
				[
					...
					function (_v3) {
						var config4 = A3($author$project$Exercises$AccountConfiguration, $author$project$Theme$Dark, false, true);
						var config3 = A3($author$project$Exercises$AccountConfiguration, $author$project$Theme$Dark, false, false);
						var config2 = A3($author$project$Exercises$AccountConfiguration, $author$project$Theme$Dark, true, false);
						var config1 = A3($author$project$Exercises$AccountConfiguration, $author$project$Theme$Light, true, false);
						return A2(
							$elm_explorations$test$Expect$equal,
							_List_fromArray(
								[config2, config2, config3, config4]),
							$author$project$Exercises$changePreferenceToDarkTheme(
								_List_fromArray(
...
var $author$project$Test$Generated$Main$main = A2(
	$author$project$Test$Runner$Node$run,
	{
		...
		paths: _List_fromArray(
			['D:\\PF\\LAB5\\tests\\DateTests.elm', 'D:\\PF\\LAB5\\tests\\Example.elm', 'D:\\PF\\LAB5\\tests\\ExerciseTests.elm', 'D:\\PF\\LAB5\\tests\\FirstTest.elm', 'D:\\PF\\LAB5\\tests\\OrganizedTests.elm']),
		processes: 8,
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$author$project$Test$Runner$Node$check($author$project$FirstTest$emptyListTakeTest)
				])),
			_Utils_Tuple2(
			'OrganizedTests',
			_List_fromArray(
				[
					$author$project$Test$Runner$Node$check($author$project$OrganizedTests$listTests)
				]))
		]));
_Platform_export({'Test':{'Generated':{'Main':{'init':$author$project$Test$Generated$Main$main($elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "\\\\.\\pipe\\elm_test-9380-1";
var net = require('net'),
