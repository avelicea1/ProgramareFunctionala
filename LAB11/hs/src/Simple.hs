...
--ex 11.2.1
string' :: String -> Parser String
string' "" = pMap (\_ -> "") (Parser (\input -> Success () input))
string' (c:cs) = pMap (\(a, b) -> a : b) (andThen (char c) (string' cs))
...
pThen parserA parserB = pMap (\(x, y) -> y) (andThen parserA parserB)


sepBy :: Parser a -> Parser b -> Parser [b]
sepBy parserA parserB= pMap id (sepBy parserA parserB)

between :: Parser a -> Parser b -> Parser c -> Parser c
between pHd pTl p=  pThen pHd (pThen p (pMap (\(_, y) -> y) p))